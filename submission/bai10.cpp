// Accepted: https://oj.vnoi.info/problem/qbsquare
...
void approach1()
{
  ...
  for (int i = 1; i < m; ++i)
  {
    for (int j = 1; j < n; ++j)
    {
      ...
      {
        f[i][j] = min(f[i - 1][j], min(f[i][j - 1], f[i - 1][j - 1])) + 1; // DON't define min function, use the predefined min() function in library <algorithm>
      }
      else
      {
        f[i][j] = 1;
